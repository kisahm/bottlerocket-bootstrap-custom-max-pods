...
function print_help {
  ...
  echo "--custom-cni <cilium> Use this if you use other CNIs like Cilium"
  echo "--cilium-first-interface-index <0/1/2>"
}
...
while [[ $# -gt 0 ]]; do
  ...
  case $key in
    ...
    --custom-cni)
      ...
      shift
      ;;
    --cilium-first-interface-index)
      CILIUM_FIRST_INDEX_INTERFACE=$2
      shift
      ;;
    *)                   # unknown option
...
CUSTOM_CNI="${CUSTOM_CNI:-}"
CILIUM_FIRST_INDEX_INTERFACE="${CILIUM_FIRST_INDEX_INTERFACE:-0}"
...
if [ "$CUSTOM_CNI" = "cilium" ] ; then
  if [ "$CNI_PREFIX_DELEGATION_ENABLED" = false ] ; then
    enis_for_pods=$(($enis_for_pods - $CILIUM_FIRST_INDEX_INTERFACE))    
  fi
fi
